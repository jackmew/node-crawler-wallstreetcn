...
const { dateTimeTransform } = require('../utils/stringUtils')
...
async function run (newsTypes, numOfRecord) {
    ...
    $('.news-main .article').each((idx, ele) => {
        ...
        if (link.indexOf('articles') === 1 && image) {
            news.push({
                ...
                content: $(ele).find('a.content').text(),
                userName: $(ele).find('div.user-name').text(),
...
async function start (newsTypes = 'global', numOfRecord = 5) {
    try {
        const { news, nightmare } = await run(newsTypes, numOfRecord)
        await nightmare.end()
        console.log(news)
    } catch (err) {
        console.log(err)
...
const newsTypes = ['global', 'shares', 'bonds', 'commodities', 'forex', 'enterprise', 
'economy', 'charts', 'china', 'us', 'europe', 'japan']
